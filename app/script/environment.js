...
class Environment // Окружение
{
	...
	init() // Инициализация приложения (окружения)
	{
		...
		let file_temp = file.addItem('Создать из шаблона', function () { cur.new_graph(); cur.import_gml(default_graph[0]); });
		file_temp.addItem('Не нагруженный граф', function () { cur.new_graph(); cur.import_gml(default_graph[2]); });
		file_temp.addItem('Нагруженный граф', function () { cur.new_graph(); cur.import_gml(default_graph[1]); });
		file_temp.addItem('Ориентированный граф', function () { cur.new_graph(); cur.import_gml(default_graph[3]); });
		file_temp.addItem('Нагруженный ориентированный граф', function () { cur.new_graph(); cur.import_gml(default_graph[0]); });
		...
		gcanv.addEventListener('drop', function (event) { obj.canvas_file_dropped(event); event.preventDefault();}, false);
		gcanv.addEventListener('dragover', function (event) { event.preventDefault();}, false);
		gcanv.addEventListener('dragleave', function (event) { event.preventDefault();}, false);
		...
		gload.addEventListener('change', function (event) { obj.canvas_loader_changed(event); }, false);
	...
	canvas_loader_changed(e) {
		this.load_files(gload.files);
	}

	canvas_file_dropped(e) {
		e.preventDefault();
		this.load_files(e.dataTransfer.files);
	}
	...
	load_files(files) {
		for (let file of files) {
			...
			reader.addEventListener('load', function (e) {
				...
				obj.new_graph();
				obj.import_gml(fileAsText);
				if (fileName) {
					obj.change_name(fileName);
					obj.change_title(fileName);
				}
			}, false);
